<?php
/**
 * Formulário de criação / edição de item do cardápio (com imagem).
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Remoção de imagem solicitada
    $removeImg = isset($_POST['remove_image']);

    // Upload de nova imagem
    $newImage = null;
    try {
        $newImage = handle_image_upload($_FILES['imagem_file'] ?? []);
    } catch (Throwable $ex) {
        $error = $ex->getMessage();
    }
...
admin_header($id ? 'Editar item' : 'Novo item', 'cardapio');
?>
...
<form class="form" method="post" enctype="multipart/form-data"
    ...
    <div class="field">
        ...
        <input id="nome" name="nome" type="text" required maxlength="120" value="<?= e($form['nome']) ?>">
    </div>
    ...
    <div class="field">
        ...
        <?php if (!empty($form['imagem'])): ?>
            <div class="img-preview">
                <img src="<?= e($form['imagem']) ?>" alt="Imagem atual">
                <label class="check">
                    <input type="checkbox" name="remove_image" value="1"> Remover imagem atual
                </label>
            </div>
        <?php endif; ?>
        <input id="imagem_file" name="imagem_file" type="file" accept="image/jpeg,image/png,image/webp,image/gif">
        <p class="field__hint">JPG, PNG, WEBP ou GIF · máx. 4 MB. <?= !empty($form['imagem']) ? 'Enviar uma nova substitui a atual.' : '' ?></p>
    </div>
        ...
        <div class="field">
            ...
            <select id="categoria" name="categoria">
                <?php foreach ($cats as $slug => $label): ?>
                    <option value="<?= e($slug) ?>" <?= $form['categoria'] === $slug ? 'selected' : '' ?>><?= e($label) ?></option>
                <?php endforeach; ?>
            </select>
            <?php if (!$cats): ?><p class="field__hint">Crie uma categoria antes em "Categorias".</p><?php endif; ?>
        </div>
        <div class="field">
            <label for="preco">Preço</label>
            <input id="preco" name="preco" type="text" required inputmode="decimal" placeholder="32,00" value="<?= e($form['preco']) ?>">
            <p class="field__hint">Digite só o valor (ex.: 32,00).</p>
        </div>
    </div>
        ...
        <div class="field">
            ...
            <select id="badge" name="badge">
                ...
                <?php foreach ($badges as $b): ?>
                    <option value="<?= e($b) ?>" <?= $form['badge'] === $b ? 'selected' : '' ?>><?= e($b) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="field">
            <label for="ordem">Ordem de exibição</label>
            <input id="ordem" name="ordem" type="number" min="0" step="1" value="<?= (int) $form['ordem'] ?>">
    ...
    <div class="form-actions">
        <button class="btn btn--primary" type="submit"><?= $id ? 'Salvar alterações' : 'Adicionar item' ?></button>
